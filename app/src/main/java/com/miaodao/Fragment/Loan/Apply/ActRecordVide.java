...
public class ActRecordVide extends Activity implements View.OnClickListener, Runnable {
...
//            isRunning = true;
//            thread = new Thread(ActRecordVide.this);
            if (!isRecording) {
                //初始化camera
                initCamera();
            }
    ...
    private class RecordHandler extends Handler {
        ...
        public RecordHandler(ActRecordVide actRecordVideo) {
        ...
        public void handleMessage(Message msg) {
            ...
            ActRecordVide act = (ActRecordVide) activityWeakReference.get();
            if (act == null) return;
...
//            mediaPlayer.prepare();
    ...
    private void openCamera() {
        ...
        try {
            recordCamera = Camera.open(1);//默认开启后置
            if (recordCamera == null) return;
            Camera.Parameters parameters = recordCamera.getParameters();
            ...
            if (focusModes.contains(Camera.Parameters.FOCUS_MODE_AUTO)) {
                ...
                recordCamera.setParameters(parameters);
            }
            setDispaly(parameters, recordCamera);
//            setCameraDisplayOrientation(this, 1, recordCamera);
            recordCamera.setParameters(parameters);

            recordCamera.setPreviewDisplay(recordHolder);
...
//        // 设置视频文件输出的路径
        File file = new File(SysSDCardCacheDir.getVideoDir().getPath(), AppConfig.getInstance().VERIFICATION_ID_VIDEO_PHONE);
