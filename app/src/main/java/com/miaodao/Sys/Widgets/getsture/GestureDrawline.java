package com.miaodao.Sys.Widgets.getsture;
...
import com.miaodao.Sys.Utils.UIToolkit;
...
public class GestureDrawline extends View {
    ...
    public GestureDrawline(Context context, List<GesturePoint> list, boolean isVerify,
        ...
        super(context);
        screenDispaly = UIToolkit.getSceenDisplay(context);
        ...
        switch (event.getAction()) {
            case MotionEvent.ACTION_DOWN:
                ...
                if (currentPoint != null) {
                    currentPoint.setPointState(AppConfig.getInstance().POINT_STATE_SELECTED);
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (pointAt == null || currentPoint.equals(pointAt) || AppConfig.getInstance().POINT_STATE_SELECTED == pointAt.getPointState()) {
                    ...
                    pointAt.setPointState(AppConfig.getInstance().POINT_STATE_SELECTED);

                    // 判断是否中间点需要选中
                    GesturePoint betweenPoint = getBetweenCheckPoint(currentPoint, pointAt);
                    if (betweenPoint != null && AppConfig.getInstance().POINT_STATE_SELECTED != betweenPoint.getPointState()) {
                        // 存在中间点并且没有被选中
                        ...
                        // 设置中间点选中
                        betweenPoint.setPointState(AppConfig.getInstance().POINT_STATE_SELECTED);
                        // 赋值当前的point;
                        currentPoint = pointAt;
                    } else {
    ...
    final class clearStateRunnable implements Runnable {
        public void run() {
            ...
            for (GesturePoint p : list) {
                p.setPointState(AppConfig.getInstance().POINT_STATE_NORMAL);
            }
            invalidate();
            isDrawEnable = true;
        }
    ...
    private void drawErrorPathTip() {
        ...
        for (Pair<GesturePoint, GesturePoint> pair : lineList) {
            pair.first.setPointState(AppConfig.getInstance().POINT_STATE_WRONG);
            pair.second.setPointState(AppConfig.getInstance().POINT_STATE_WRONG);
            canvas.drawLine(pair.first.getCenterX(), pair.first.getCenterY(),
                    pair.second.getCenterX(), pair.second.getCenterY(), paint);// 画线
        }
        invalidate();
